...
async def main():
	if m := ctx.group_msg:
		...
		if Tools.commandMatch(m, blockGroupNumber):
			return
		mhead = m.msg_head
		userQQ = mhead.SenderUin
		msg = ctx.group_msg.text
		nickname = mhead.SenderNick
		userGroup = mhead.FromUin
		if Text := mainProgram(userQQ, userGroup, msg, nickname):
			await S.text(Text)
...
def mainProgram(userQQ, userGroup, msg, nickname):
	...
	if Tools.commandMatch(msg, goodMorningInstructionSet):
		return goodMorningInformation(userQQ, userGroup, nickname)
	# Good night detection
	if Tools.commandMatch(msg, goodNightInstructionSet):
		return goodNightInformation(userQQ, userGroup, nickname)


mark_recv(main, name='早晚安', author='@xiyaowong', usage='早安~ 晚安~')
...
class Tools:
	...
	# @staticmethod
	# def sendText(msg, model=Model.SEND_DEFAULT):  # 不敢用At，暂时注释掉
